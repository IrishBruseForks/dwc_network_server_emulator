...
def calculate_crc8(inp):
    ...
    """
    crc_table = [
        0x00, 0x07, 0x0E, 0x09, 0x1C, 0x1B, 0x12, 0x15, 0x38, 0x3F, 0x36, 0x31, 0x24, 0x23, 0x2A, 0x2D, 0x70, 0x77, 0x7E, 0x79, 0x6C, 0x6B, 0x62, 0x65, 0x48, 0x4F, 0x46, 0x41, 0x54, 0x53, 0x5A, 0x5D, 0xE0, 0xE7, 0xEE, 0xE9, 0xFC, 0xFB,
        ...
        0xB2, 0xB5, 0xBC, 0xBB, 0x96, 0x91, 0x98, 0x9F, 0x8A, 0x8D, 0x84, 0x83, 0xDE, 0xD9, 0xD0, 0xD7, 0xC2, 0xC5, 0xCC, 0xCB, 0xE6, 0xE1, 0xE8, 0xEF, 0xFA, 0xFD, 0xF4, 0xF3
    ]
    crc = 0
...
def pretty_print_hex(orig_data, cols=16, sep=' '):
    ...
    while i < end:
        if i + cols < end:
            ...
            j = end
        output += line % (i, size, sep.join("%02x" % c for c in data[i:j]), "".join(chr(c) if 0x20 <= c < 0x7F else '.' for c in data[i:j]))
        i += cols
...
def dict_to_qs(d):
    ...
    ret = {
        k: base64.b64encode(v.encode("ascii")).decode("ascii").replace("=", "*")
        for k, v in d.items()
    }
